...
const ResetPasswordPage = ({ t }) => {
  ...
  return (
    <WrapAuth title={t('reset-password')} onFinish={onFinish}>
      ...
      >
        <Input.Password prefix={<Icon name="mdiLockOutline" />} placeholder={t('attributes.new-password')} />
      </Form.Item>
      <Form.Item
        name="re_new_password"
        rules={[{ required: true, message: t('messages.input', { attr: t('attributes.repeat-new-password') }) }]}
      >
        <Input.Password prefix={<Icon name="mdiLockOutline" />} placeholder={t('attributes.repeat-new-password')} />
      </Form.Item>
